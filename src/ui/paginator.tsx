...
export default function Paginator<T extends string>(props: Props<T>) {
  return (
    <nav className="flex justify-between">
      {!props.isFirst && (
        <NextAnchor href={props.prev} className="group flex items-center gap-2">
          <ArrowLeft className="stroke-brand" aria-hidden />
      ...
      {!props.isLast && (
        ...
        >
          ...
          <ArrowRight className="stroke-brand" aria-hidden />
        </NextAnchor>
      )}
    </nav>
  );
